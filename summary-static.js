(function installStaticCourseMaps(){
      ...
      <div class="static-map-shell course-one-map">
        ...
        <div class="static-map-branches">
          ${branch('01 · BASICS',bi('建立心智模型','Mental model'),[
            ...
            bi('输入 → 模型 → 预测','input → model → prediction')
          ],'blue')}
          ...
          ${branch('03 · CAPABILITY',bi('模型需要什么能力','What capability is needed'),[
            'GNN · Message Passing',
            '3D · Multimodal',
          ...
          ${branch('04 · PRACTICE',bi('真实科研案例','Real research cases'),[
            'NMRNet · uMLP',
            'Cat-KG · NOSE · Uni-XAS',
            bi('Agent · 科研循环','Agent · research loop')
          ],'coral')}
        </div>
      ...
      <div class="static-map-shell course-two-map">
        ...
        <div class="static-map-branches">
          ...
          ${branch('02 · TRAINING',bi('训练怎样推进','How training proceeds'),[
            'Batch · Epoch',
            bi('训练误差下降 ≠ 泛化更好','Lower train error ≠ better generalization'),
            bi('关注留出数据表现','Track held-out performance')
          ],'yellow')}
          ${branch('03 · HOLD OUT',bi('未来数据怎样模拟','How future data is simulated'),[
            'Train · Validation · Test',
            'Random · Scaffold · Time · External',
            bi('避免泄漏，匹配真实场景','Avoid leakage; match intended use')
          ],'sage')}
          ${branch('04 · EVALUATE',bi('怎样判断可信','How to judge credibility'),[
            'MAE · RMSE · R²',
            bi('看误差分布与失败样本','Inspect failures and error distribution'),
  ...
  if(!document.getElementById('static-course-map-style')){
    ...
    style.textContent=`
      ...
      .static-map-branch{position:relative;min-width:0;padding:15px 14px 12px;border:1.7px solid var(--ink);border-radius:13px 17px 12px 15px;background:var(--paper);box-shadow:3px 4px 0 rgba(38,51,47,.08);display:flex;flex-direction:column;justify-content:center}
      ...
      .static-map-branch ul{margin:10px 0 0;padding:0;list-style:none;display:grid;gap:8px}
      .static-map-branch li{position:relative;padding-left:12px;font-size:11.5px;line-height:1.4;color:var(--muted);font-weight:760}
