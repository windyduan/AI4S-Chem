(function installLectureRebuildPass(){
  ...
  function rebuildP13(){
    ...
    const nodes=zh()?[
      ['01','定义问题','预测什么、解释什么、优化什么'],
      ['02','准备数据','实验、计算、数据库记录'],
      ['03','选择表示','让模型看到合适的化学信息'],
      ['04','训练模型','学习输入到目标的关系'],
      ['05','独立验证','检查未见数据上的可信度'],
      ['06','产生新证据','实验 / 计算反馈下一轮']
    ]:[
      ['01','Define question','What should be predicted, explained or optimized?'],
      ['02','Prepare data','Experiments, computation and databases'],
      ['03','Choose representation','Expose the right chemical information'],
      ['04','Train model','Learn the mapping from inputs to targets'],
      ['05','Validate independently','Check credibility on unseen data'],
      ['06','Create new evidence','Experiment / computation feeds the next round']
    ];
  ...
  function rebuildP29(){
    ...
    if(!lab.dataset.ready){
      ...
      const draw=()=>{
        const story=zh()?[
          ['训练集','350 个样本'],['Batch size','每批 50 个'],['一次更新','处理完 1 批'],['一个 Epoch','7 批全部走完']
        ]:[['Training set','350 samples'],['Batch size','50 per batch'],['One update','after one batch'],['One epoch','after all 7 batches']];
      ...
      $('.p29-next',lab).addEventListener('click',()=>{step=Math.min(7,step+1);draw()});$('.p29-reset',lab).addEventListener('click',()=>{step=0;draw()});
      lab._drawP29=draw;draw();
    }else{
      $$('.p29-batch',lab).forEach((b,i)=>{const sm=$('small',b);if(sm)sm.textContent=zh()?`第 ${i+1} 批`:`Batch ${i+1}`});lab._drawP29?.();
    }
  }

  const caseNarratives={
    'case-nmrnet':{
      titleZh:'NMR 化学位移靠局域 3D 环境决定，人能不能靠规则把所有情况写完？',titleEn:'NMR shifts depend on local 3D environments — can hand-written rules cover every case?',
      labelsZh:['背景：结构决定谱学','传统方法：规则难统一','AI：学习 3D 局域环境','价值：跨体系预测'],labelsEn:['background: structure → spectrum','traditional: rules do not generalize','AI: learn local 3D environments','value: transfer across systems'],
      copyZh:['NMR chemical shift 对目标原子周围的局域化学环境非常敏感；液态与固态体系还会带来不同结构分布。','经验规则或简单描述符能覆盖部分情况，但很难把复杂三维几何与不同数据集统一起来。','SE(3)-aware 模型直接读取目标原子周围的 3D 环境，并通过预训练 + 微调学习更可迁移的结构—谱学关系。','最终输出原子级 chemical shift，并把液态与固态任务放到统一 benchmark 中比较。这体现了 3D representation 真正进入实验表征问题。']
    },
    'case-nose':{
      titleZh:'气味不是只看分子结构：结构、嗅觉受体和人类语言怎样被放到一起？',titleEn:'Odor is not structure alone — how can molecules, receptors and language be connected?',
      labelsZh:['背景：气味跨三种信息','传统方法：各自分开建模','AI：分子作为共同枢纽','价值：统一嗅觉表示'],labelsEn:['background: three information sources','traditional: modeled separately','AI: molecule as shared hub','value: unified olfactory representation'],
      copyZh:['气味感知同时涉及化学结构、生物受体响应和人类语言描述，任何单一模态都只看到链条的一部分。','过去常把分子—气味、分子—受体分别建模；真正同时包含三者的完整数据又很稀缺。','NOSE 让分子表示成为中心枢纽，分别接入受体序列与气味语义，并用正交约束减少两类信息相互覆盖。','得到的统一表示可以支持感知预测、跨模态检索与零样本泛化。AI 在这里解决的是“信息彼此割裂”的问题。']
    },
    'case-unixas':{
      titleZh:'XAS 给的是一条谱线，但真正想知道的是局域 3D 结构：这个逆问题怎么做？',titleEn:'XAS gives a spectrum, but we want local 3D structure — how can this inverse problem be solved?',
      labelsZh:['背景：谱线间接编码结构','传统方法：逆推不唯一','AI：跨模态共享空间','价值：谱 ↔ 结构双向'],labelsEn:['background: spectrum encodes structure indirectly','traditional: inverse mapping is ambiguous','AI: shared cross-modal space','value: spectrum ↔ structure'],
      copyZh:['XAS 是局域结构的实验投影，但观测到的是一维谱线，真正关心的却是三维原子环境。','从谱线直接逆推出结构通常并不唯一，还需要大量物理建模、候选假设与比较。','Uni-XAS 把 XAS 光谱和局域 3D 结构编码到共享表示空间，让相互检索、匹配与条件生成变成统一任务。','同一个表示框架可以做结构—光谱检索、光谱预测和光谱条件下的 3D 生成，让实验信号更直接地连接结构解释。']
    },
    'case-electroplating':{
      titleZh:'配方研发同时需要多个模型、工具和实验，人怎样避免在流程之间来回搬运？',titleEn:'Formulation R&D needs many models, tools and experiments — how can the workflow be coordinated?',
      labelsZh:['背景：多任务耦合','传统方法：人工串流程','AI：Agent 编排专业模型','价值：形成实验闭环'],labelsEn:['background: coupled tasks','traditional: manual orchestration','AI: agent coordinates specialists','value: closed experimental loop'],
      copyZh:['电镀添加剂研发同时涉及吸附、扩散、分子设计、CVS 响应与实验验证，一个模型不可能包办所有问题。','传统做法需要研究者在不同模型、代码、数据库和实验之间手工搬运目标与结果，流程长且容易丢失上下文。','Agent 不替代物理化学模型，而是理解研发目标、选择专业模型与工具、组织调用顺序并保留中间证据。','候选经过预测与筛选进入实验，实验结果再反馈下一轮。价值在于让多个专业能力形成可迭代研发闭环。']
    }
  };
