(function enforceCourseOrder(){
  ...
  const retiredIds=['discovery-story','prediction-loss-screen','data-split','generalization-curves','unseen-vocabulary'];

  function removeRetired(){retiredIds.forEach(id=>document.getElementById(id)?.remove())}
  ...
  const secondIds=['equation-screen','train','training-playground-screen','batch-epoch-screen','holdout-screen','play','split-scenarios','evaluation-traps','metric-lab-screen','r2-baseline-screen','trust-zone-screen','generalization-review','generalization-checkpoint'];
