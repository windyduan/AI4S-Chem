(function installP40ReviewFix(){
  ...
  const copy={
    zh:{
      ...
      rule:'判断顺序：先看数据怎么切，再看模型错在哪里，最后问这个候选是否落在验证过的范围内'
    },
    en:{
      no:'02L / REVIEW',
      title:'Before trusting a model result, ask these three questions',
      cards:[
      ...
      rule:'Order of judgment: inspect the split, inspect how the model fails, then ask whether this candidate lies in a validated regime'
    }
  };

  function apply(){
    const c=zh()?copy.zh:copy.en;
    const number=section.querySelector('.section-no');if(number)number.textContent=c.no;
    section.querySelector('.story-kicker')?.remove();
    section.querySelector('.story-copy .lead')?.remove();
    const title=section.querySelector('.story-copy h2');if(title)title.textContent=c.title;
    const cards=[...section.querySelectorAll('.gen-review-card')];
    cards.forEach((card,i)=>{const d=c.cards[i];if(!d)return;const small=card.querySelector('small'),strong=card.querySelector('strong'),p=card.querySelector('p');if(small)small.textContent=d[0];if(strong)strong.textContent=d[1];if(p)p.textContent=d[2]});
    const rule=section.querySelector('.gen-review-rule');if(rule)rule.textContent=c.rule;
  }
  ...
  if(!document.getElementById('p40-review-style')){
    ...
    style.textContent=`
      ...
      #generalization-review .gen-review-rule{margin-top:22px;font-size:clamp(14px,1.22vw,17px);font-weight:780;color:#5c463d;line-height:1.45}
