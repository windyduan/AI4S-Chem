(function installRepresentationP7Fixes(){
  ...
  // P7 follows the role-map screen in the reordered course.
  const sectionNo=rep.querySelector('.section-no');
  if(sectionNo)sectionNo.textContent='01D / REPRESENTATION';

  // Give the Chinese question enough horizontal room to stay on one line on presentation screens.
  const title=rep.querySelector('h2');
  if(title)title.classList.add('p7-title');

  const style=document.createElement('style');
  style.textContent=`
    #represent{max-width:1280px!important}
    #represent .p7-title{max-width:none!important;width:100%;font-size:clamp(31px,3.65vw,50px);white-space:nowrap}
    #represent .rep-explicit-smiles{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:clamp(18px,2.45vw,31px);line-height:1.45;word-break:break-all;max-width:900px}
    ...
    #represent .descriptor-grid{grid-template-columns:repeat(4,minmax(112px,1fr));gap:12px;width:min(780px,100%)}
    #represent .descriptor-chip{display:grid;grid-template-columns:38px 1fr;grid-template-rows:auto auto;column-gap:10px;align-items:center;text-align:left;padding:13px 12px}
    #represent .descriptor-symbol{grid-row:1/3;display:grid;place-items:center;width:36px;height:36px;border:1.5px solid var(--ink);border-radius:50%;background:var(--paper);font-size:13px;font-weight:950;line-height:1;white-space:nowrap;overflow:hidden}
    #represent .descriptor-symbol.hbd-symbol{font-size:10.5px;letter-spacing:-.07em}
    #represent .descriptor-chip small{font-size:9px;letter-spacing:.08em}#represent .descriptor-chip strong{font-size:18px}
    ...
    #represent .rep-3d-wrap{position:relative;width:min(660px,100%);height:198px;margin:0 auto;display:grid;place-items:start center;overflow:hidden}
    ...
    #represent .rep-3d svg{display:block;width:min(530px,86%);height:auto;max-height:186px;overflow:hidden;animation:rep3dFloat 6.5s ease-in-out infinite;transform-origin:center}
    @keyframes rep3dFloat{0%,100%{transform:translateY(-4px)}50%{transform:translateY(-7px)}}
    ...
    @media(max-width:900px){#represent .p7-title{white-space:normal;font-size:clamp(30px,5vw,45px)}}
    @media(max-width:760px){#represent .descriptor-grid{grid-template-columns:1fr 1fr}#represent #rep-stage.p7-three-active{height:196px;padding:2px 6px}#represent .rep-3d-wrap{height:186px}#represent .rep-3d{height:174px}#represent .rep-3d svg{animation:none;width:88%;max-height:164px}}
  `;
  ...
  const moleculeHTML=`<div class="molecule-sketch">CH<sub>3</sub>—CH<sub>2</sub>—OH</div>`;
  const smilesHTML=`<div class="rep-explicit-smiles">[H]C([H])([H])C([H])([H])O[H]</div>`;
  const fingerprintHTML=`<div class="fingerprint">00100101000110101010010001100101</div>`;
  const ethanolGraph=`<svg class="ethanol-graph" viewBox="0 0 760 330" role="img" aria-label="Ethanol molecular graph with all hydrogens shown">
    <g>
  ...
  const descriptorHTML=`<div class="descriptor-grid">
    ...
    <div class="descriptor-chip"><span class="descriptor-symbol hbd-symbol">H···</span><small>HBD</small><strong>1</strong></div>
    <div class="descriptor-chip"><span class="descriptor-symbol">◎</span><small>TPSA</small><strong>20.23</strong></div>
  </div>`;
  const threeHTML=`<div class="rep-3d-wrap"><div class="rep-3d"><svg viewBox="0 0 700 300" role="img" aria-label="Ethanol ball-and-stick model with all atoms and hydrogens shown">
    <defs>
  ...
  intercept(molecule,moleculeHTML);
  intercept(smiles,smilesHTML);
  intercept(fingerprint,fingerprintHTML);
  intercept(graph,ethanolGraph);
  intercept(descriptor,descriptorHTML);
  intercept(three,threeHTML);

  // Normalize the initial active tab too, not only content shown after a click.
  const active=tabs.querySelector('.rep-tab.active')||molecule;
  const initial={molecule:moleculeHTML,smiles:smilesHTML,fingerprint:fingerprintHTML,graph:ethanolGraph,descriptor:descriptorHTML,'three-d':threeHTML};
  if(active&&initial[active.dataset.rep])activate(active,initial[active.dataset.rep]);
})();
